...
r = 0.3; % Decaimento Gaussiano
m = 3.4; % Grau de incerteza
gama = 0.6; % Peso multivariavel
...
% hold on
% plot(centros(end-1,:),centros(end,:),'k*',xc(end-1,:),xc(end,:),'r*','LineWidth',1)
% xm = (-0.1:0.005:1.1)';
% xmnn = xm*(max_x(end-1)-min_x(end-1)) + min_x(end-1);
% ym = (-0.1:0.005:1.1)';
% ymnn = ym*(max_x(end)-min_x(end)) + min_x(end);
% for l = 1:R
%     l
%     for i = 1:length(xm)
%         for j = 1:length(ym)
%             dist = sqrt(([xcn(1:end-2,l);xm(i);ym(j)]-xcn(:,l))'*det(IF(:,:,l))^(-1/n)*IF(:,:,l)*...
%                 ([xcn(1:end-2,l);xm(i);ym(j)]-xcn(:,l)));
%             Dm(i,j,l) = exp(dist)-1;
%         end
%     end
...
% figure
% plot(x(end-1,:),x(6,:),'.','Color',[0.8 0.7 1],'LineWidth',1)
% hold on
% plot(xc(end-1,:),xc(6,:),'r*','LineWidth',1)
% xm = (-0.1:0.005:1.1)';
% xmnn = xm*(max_x(end-1)-min_x(end-1)) + min_x(end-1);
% ym = (-0.1:0.005:1.1)';
% ymnn = ym*(max_x(end)-min_x(end)) + min_x(end);
...
%     for j = 1:length(ym)
%         for l = 1:R
%             Dm(i,j,l) = exp(sqrt(([xcn(1:end-2,l);xm(i);ym(j)]-xcn(:,l))'*det(IF(:,:,l))^(-1/n)*IF(:,:,l)*...
%                 ([xcn(1:end-2,l);xm(i);ym(j)]-xcn(:,l))))-1;
%         end
...
% end
% for l = 1:R
%     contour(xmnn,ymnn,mum(:,:,l)',0.7*[1,1],'r');
%     contour(xmnn,ymnn,mum(:,:,l)',0.5*[1,1],'b');
%     contour(xmnn,ymnn,mum(:,:,l)',0.3*[1,1],'k');
% end
...
for i = 1:qt_rotulos % Para todos clusters encontrados
    for j = 1:R % Para todos os clusters reais
        d(j,i) = norm(xcn(:,j)-centros_(:,i)); % Cada coluna tem as distâncias do real para os meus
    end
    d(:,i) = sort(d(:,i));
end
indice_erro_centro = sqrt(mean(d(1,:).^2))
%
% u__ = (u_-min_x(1))/(max_x(1)-min_x(1));
% y__ = (y_-min_x(2))/(max_x(2)-min_x(2));
% for i = 1:qt_rotulos % Para todos clusters encontrados
%     for j = 1:R % Para todos os clusters reais
%         d(j,i) = norm(xcn(:,j)-[u__(i);y__(i)]); % Cada coluna tem as distâncias do real para os meus
%     end
%     dist_f(i,1) = min(d(:,i));
% end
...
% end
% ix = [];
% for i = 1:OTIMOSDIM06.Q
%     if OTIMOSDIM06.indvd_ord(i,1) > 0.069 && OTIMOSDIM06.indvd_ord(i,1) < 0.071
%         ix = [ix;i];
%     end
% end
% load('OTIMOSDIM06.mat')
% tudo = [OTIMOSDIM06.indices_ord, OTIMOSDIM06.indvd_ord];