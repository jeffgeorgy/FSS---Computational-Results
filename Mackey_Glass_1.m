...
load('T.mat')
load('X.mat'); % Carrega os dados da serie
tk = T; % Vetor de amostras
% Amostras em regime permanente
max_X = 1.35; min_X = 0.2; % Valor maximo e minimo da entrada para normalizacao
X_n = (X-min_X)/(max_X-min_X); % normalizando
%% Inicializa as variaveis do Algoritmo ETS
% Parâmetros do algoritmo
r = 0.8; % raio de influencia de um cluster
epsilon = 0.2; % Fator de raio de cluster
m = 2.7; %Ponderamento exponencial para calculo de pertinencias
gama = 0.7; % Ponderamento das informacoes no potencial
s = 0.8; % Fator de sensibilidade para determinacao de candidato
...
y = X(2:end);
yn = (y-min_X)/(max_X-min_X);
% u = 0.05*randn(length(y),1);
load('u.mat')
...
figure
plot(xn(1,:),xn(2,:),'c*',xcn{end}(1,:),xcn{end}(2,:),'r*')