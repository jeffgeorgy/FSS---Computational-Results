...
r = 0.44; % Decaimento Gaussiano
m = 3; % Grau de incerteza
gama = 0; % Peso multivariavel
...
%% Parametros ODIK/ERA
Ni = 25; % Instante de inicializacao
nent = length(u(1,:));
nsai = length(y(1,:));
p_pm = 2;
v = [u';y'];
...
xlabel('$u_k^{2}$','Interpreter','latex')
...
xlabel('$k$','Interpreter','latex')
ylabel('$u_k^1$','Interpreter','latex')
grid on
xlim([0 50])
xticks(0:5:50)
subplot(2,1,2)
...
xlabel('$k$','Interpreter','latex')
ylabel('$u_k^2$','Interpreter','latex')
grid on
xlim([0 50])
xticks(0:5:50)
