...
clc
close all
dados = load('unbalance.txt');
centros = load('unbalance-gt.txt');
...
r = 0.12; % Decaimento Gaussiano
m = 2.4; % Grau de incerteza
gama = 0.49; % Peso multivariavel
...
hold on
plot(u_,y_,'k*',xc{end}(1,:),xc{end}(2,:),'r*','LineWidth',1)
xm = (-0.1:0.005:1.1)';
...
legend("Dados $\mathbf{x}_k$","Centros de Grupo Reais","Centros $\mathbf{c}_k^i$ Identificados","Prot\'{o}tipos dos Grupos",'Interpreter','latex')
...
legend("Dados $\mathbf{x}_k$","Centros $\mathbf{c}_k^i$ no Processo",...
    "Centros de Grupo Reais","Centros $\mathbf{c}_k^i$ Finais",'Interpreter','latex')
xlabel('$x_k^1$','Interpreter','latex')
ylabel('$x_k^2$','Interpreter','latex')
grid on
...
for l = 1:R(end)
    contour(xmnn,ymnn,mum(:,:,l)',0.7*[1,1],'k--','LineWidth',1);
    contour(xmnn,ymnn,mum(:,:,l)',0.5*[1,1],'b--','LineWidth',1);
    contour(xmnn,ymnn,mum(:,:,l)',0.3*[1,1],'g--','LineWidth',1);
end
legend("Dados $\mathbf{x}_k$","Centros de Grupo $\mathbf{c}_k^i$",...
    "N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,7","N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,5"...
    ,"N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,3",'Interpreter','latex')
xlabel('$x_k^1$','Interpreter','latex')
ylabel('$x_k^2$','Interpreter','latex')
grid on
