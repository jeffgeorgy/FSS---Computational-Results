...
load('u10.mat')
load('y10.mat'); % Carrega os dados da serie
for i = 1:length(u(1,:))
    mx = max(u(:,i));
    mn = min(u(:,i));
    if mx == mn
...
r = 2.14; % Decaimento Gaussiano
m = 2.2; % Grau de incerteza
gama = 0.3; % Peso multivariavel
...
%% Parametros ODIK/ERA
Ni = 900; % Instante de inicializacao
nent = length(u(1,:));
nsai = length(y(1,:));
p_pm = 2;
v = [u';y'];
...
xlabel('$u_k^{10}$','Interpreter','latex')
...
figure
subplot(3,1,1)
plot(t,u(:,8),'b','LineWidth',1)
xlabel('$k$','Interpreter','latex')
ylabel('$u_k^8$','Interpreter','latex')
grid on
xlim([0 2900])
xticks(0:300:3000)
subplot(3,1,2)
plot(t,u(:,9),'b','LineWidth',1)
xlabel('$k$','Interpreter','latex')
ylabel('$u_k^9$','Interpreter','latex')
grid on
xlim([0 2900])
xticks(0:300:3000)
subplot(3,1,3)
plot(t,u(:,10),'b','LineWidth',1)
xlabel('$k$','Interpreter','latex')
ylabel('$u_k^{10}$','Interpreter','latex')
grid on
xlim([0 2900])
xticks(0:300:3000)
