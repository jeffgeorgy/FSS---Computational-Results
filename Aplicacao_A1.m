...
u = dados(:,1);
y = dados(:,2);
max_u = 7e4; min_u = 0; % Valor maximo e minimo da entrada para normalizacao
max_y = 6.5e4; min_y = 0; % Valor maximo e minimo da saida para normalizacao
...
epsilon = 0.365; % Fator de raio de cluster
m = 2; %Ponderamento exponencial para calculo de pertinencias 2.8
gama = 0.78; % Ponderamento das informacoes no potencial 0.775
s = 0.8; % Fator de sensibilidade para determinacao de candidato 0.81
...
% xlim([0 max_u])
axis equal
axis([-0.25e4 6.75e4 1.5e4 8.5e4])
...
% end
% for l = 1:R
%     contour(xm,ym,mum(:,:,l)',0.7*[1,1],'r');
%     contour(xm,ym,mum(:,:,l)',0.45*[1,1],'b');
%     contour(xm,ym,mum(:,:,l)',0.2*[1,1],'k');
% end
% legend("Samples", "Clusters centers","\mu = 0.7",...
%     "\mu = 0.45","\mu = 0.2");
% xlabel('Feature 1')
% ylabel('Feature 2')
% ylim([0.35 1.1])
...
for i = 1:R
    for j = 1:R
        d(j,i) = norm(xcn{end}(:,i)-[u__(j);y__(j)]);
    end
    d(:,i) = sort(d(:,i));
end
...
% 
% % xx = 13;
% % for i = 150*(xx-1)+1:150*xx
% %     plot(u_n(i),y_n(i),'g*')
% % end
% % 
% % xx = 13;
% % plot((150*(xx-1)+1:150*xx)',P(150*(xx-1)+1:150*xx),'g')