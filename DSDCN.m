...
Ni = 250; % Instante de inicializacao
nent = length(u(1,:));
nsai = length(y(1,:));
p_pm = 2;
v = [u';y'];
...
% end
figure
plot(u,y,'c*',u_,y_,'k--',xc(1,:),xc(2,:),'r*','LineWidth',1)
hold on
% xm = (-0.1:0.005:1.1)';
% ym = (-0.1:0.005:1.1)';
% for l = 1:R
...
legend('Vectors of data','Static curve','Centers of clusters')
...
% figure
% plot(un,yn,'c*',xcn{end}(1,:),xcn{end}(2,:),'r*','LineWidth',1)
% hold on
% xm = (-0.1:0.005:1.1)';
% ym = (-0.1:0.005:1.1)';
% for i = 1:length(xm)
