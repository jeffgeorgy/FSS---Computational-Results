...
dados = load('dim064.txt');
dados = dados';
centros = load('c-dim064.txt');
centros = centros';
...
r = 0.5; % Decaimento Gaussiano
m = 4; % Grau de incerteza
gama = 0.45; % Peso multivariavel
tic % Inicializa a contagem de tempo
% Variaveis do algoritmo
k = 1; % Amostra inicial
R(1) = 1; % Numero de regras inicial
x = dados; % Vetor de dados
n = length(x(:,1)); % Numero de coordenadas
xn = dados_; % Primeiro dado normalizado
...
ix1 = length(dados(:,1))-1;
ix2 = length(dados(:,1));
% %% Figura 1 (a) - Centros e Protótipos
% figure
% plot(x(end-1,:),x(end,:),'c*','LineWidth',1)
% hold on
% plot(centros(end-1,:),centros(end,:),'ko',xc{end}(end-1,:),xc{end}(end,:),'r*','LineWidth',1)
% xm = (-0.1:0.005:1.1)';
% xmnn = xm*(max(x(end-1,:))-min(x(end-1,:))) + min(x(end-1,:));
% ym = (-0.1:0.005:1.1)';
% ymnn = ym*(max(x(end,:))-min(x(end,:))) + min(x(end,:));
% for l = 1:R(end)
%     l
%     for i = 1:length(xm)
%         for j = 1:length(ym)
%             dist = sqrt(([xcn{end}(1:end-2,l);xm(i);ym(j)]-xcn{end}(:,l))'*det(F{end}(:,:,l))^(1/n)*inv(F{end}(:,:,l))*...
%                 ([xcn{end}(1:end-2,l);xm(i);ym(j)]-xcn{end}(:,l)));
%             Dm(i,j,l) = exp(dist)-1;
%         end
%     end
%     contour(xmnn,ymnn,Dm(:,:,l)',0.05*[1,1],'r','LineWidth',1);
% end
% 
% legend("Vectors of Data","Real Centers","Achieved Centers","Prototypes",'Interpreter','latex')
% xlabel(strcat("$x_k^{",num2str(ix1),"}$"),'Interpreter','latex')
% ylabel(strcat("$x_k^{",num2str(ix2),"}$"),'Interpreter','latex')
% grid on
% yticks(20:20:240)
% xticks(20:20:240)
...
% figure
% for i = 1:R(end)
%     plot(DA{i}(end-1,:),DA{i}(end,:),'.','Color',colors(i,:),'LineWidth',1);
%     if i == 1
%         hold on
%     end
%     leg{i,1} = strcat("Grupo $\mathbf{\Theta}_k^{",num2str(i),"}$");
% end
% plot(xc{end}(end-1,:),xc{end}(end,:),'k*','LineWidth',1)
...
% figure
% plot(x(end-1,:),x(end,:),'.','Color',[0.8 0.7 1],'LineWidth',1)
% hold on
% plot(centros(end-1,:),centros(end,:),'k*','LineWidth',1)
% for i = 1:R(end)
%     plot(deslocamento{i}(end-1,2:end),deslocamento{i}(end,2:end),'b*',...
%         deslocamento{i}(end-1,1),deslocamento{i}(end,1),'g*',...
%         xc{end}(end-1,i),xc{end}(end,i),'r*','LineWidth',1);
% end
% plot(centros(end-1,:),centros(end,:),'k*','LineWidth',1)
% for i = 1:R(end)
%     plot(xc{end}(end-1,i),xc{end}(end,i),'r*','LineWidth',1);
% end
...
plot(centros(end-1,:),centros(end,:),'ko','LineWidth',1)
...
%     for j = 1:length(ym)
%         for l = 1:R(end)
%             Dm(i,j,l) = exp(sqrt(([xcn{end}(1:end-2,l);xm(i);ym(j)]-xcn{end}(:,l))'*det(F{end}(:,:,l))^(1/n)*inv(F{end}(:,:,l))*...
%                 ([xcn{end}(1:end-2,l);xm(i);ym(j)]-xcn{end}(:,l))))-1;
%         end
...
% end
% for l = 1:R(end)
%     contour(xmnn,ymnn,mum(:,:,l)',0.4*[1,1],'r','LineWidth',1);
%     contour(xmnn,ymnn,mum(:,:,l)',0.3*[1,1],'b','LineWidth',1);
%     contour(xmnn,ymnn,mum(:,:,l)',0.2*[1,1],'k','LineWidth',1);
% end
% legend("Vectors of Data","Centers of Clusters",...
%     "$\mu$ = 0.4","$\mu$ = 0.3"...
%     ,"$\mu$ = 0.2",'Interpreter','latex')
% xlabel(strcat("$x_k^{",num2str(ix1),"}$"),'Interpreter','latex')
% ylabel(strcat("$x_k^{",num2str(ix2),"}$"),'Interpreter','latex')
% grid on
% yticks(20:20:240)
...
% figure
% subplot(2,1,1)
% plot(t,x(end-1,:),'k','LineWidth',1)
% xlabel('$k$','Interpreter','latex')
% ylabel(strcat("$x_k^{",num2str(ix1),"}$"),'Interpreter','latex')
% xlim([0 1024])
% xticks(0:100:1000)
% grid on
% yticks_ = get(gca, 'YTick'); % Obter valores dos ticks no eixo Y
...
% xlim([0 1024])
% xticks(0:100:1000)
% %% Figura 2 (d) - Potenciais dos centros no processo
% figure
% PotC = zeros(R(end),N);
% for k = 1:N
%     if length(Pc{k}) == R(end)
%         PotC(:,k) = Pc{k};
%     elseif length(Pc{k}) < R(end)
%         PotC(:,k) = [Pc{k};zeros(R(end)-length(Pc{k}),1)];
%     else
%         PotC(:,k) = Pc{k}(1:R(end));
%     end
% end
...
%     if length(S{k}) == R(end)
%         SC(:,k) = S{k};
%     elseif length(S{k}) < R(end)
%         SC(:,k) = [S{k};zeros(R(end)-length(S{k}),1)];
%     else
%         SC(:,k) = S{k}(1:R(end));
%     end
% end
...
% figure
% epsilonC = zeros(R(end),N);
% for k = 1:N
%     if length(epsilon{k}) == R(end)
%         epsilonC(:,k) = epsilon{k};
%     elseif length(Pc{k}) < R(end)
%         epsilonC(:,k) = [epsilon{k};zeros(R(end)-length(epsilon{k}),1)];
%     else
%         epsilonC(:,k) = epsilon{k}(1:R(end));
%     end
% end
% for i = 1:R(end)
...
% figure
% for i = 1:R(end)
%     plot(dadosg{i}(end-1,:),dadosg{i}(end,:),'.','Color',colors(i,:),'LineWidth',1);
%     if i == 1
%         hold on
%     end
%     leg{i,1} = strcat("Grupo $\mathbf{\Theta}_k^{",num2str(i),"}$");
...
% for i = 1:n
%     centros_(i,:) = (centros(i,:)-min_x{end}(i))/(max_x{end}(i)-min_x{end}(i));
% end
% for i = 1:qt_rotulos % Para todos clusters encontrados
%     for j = 1:R(end) % Para todos os clusters reais
%         d(j,i) = norm(xcn{end}(:,j)-centros_(:,i)); % Cada coluna tem as distâncias do real para os meus
%     end
%     d(:,i) = sort(d(:,i));
% end
