...
clc
close all
dados = load('S2.txt');  % Carrega o conjunto de dados
centros = load('s2-cb.txt'); % Carrega os centros reais de clusters
...
for l = 1:R(end)
    contour(xmnn,ymnn,mum(:,:,l)',0.9*[1,1],'k--','LineWidth',1);
    contour(xmnn,ymnn,mum(:,:,l)',0.7*[1,1],'b--','LineWidth',1);
    contour(xmnn,ymnn,mum(:,:,l)',0.5*[1,1],'g--','LineWidth',1);
end
legend("Dados $\mathbf{x}_k$","Centros de Grupo $\mathbf{c}_k^i$",...
    "N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,9","N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,7"...
    ,"N\'{i}veis de Contorno $\mu_{k,N}^i$ = 0,5",'Interpreter','latex')
xlabel('$x_k^1$','Interpreter','latex')
ylabel('$x_k^2$','Interpreter','latex')
grid on
