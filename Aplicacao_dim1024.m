...
dados = load('dim1024.txt');
dados = dados';
centros = load('c-dim1024.txt');
centros = centros';
...
r = 7; % raio de influencia de um cluster
...
for k = 2:N
    ...
    for i = 1:R % Medicao da distancia adaptativa do dado atual para os centros
        ...
        %     (xn(:,k)-xcn{k}(:,i)));
        dist = sqrt((xn(:,k)-xcn{k}(:,i))'*(xn(:,k)-xcn{k}(:,i)));
    ...
    if P(k) > s*max(Pc{k}(:,1)) % Se o potencial do dado atual for maior
        ...
        if dmin < epsilon*r
            ...
            dist = sqrt((xn(:,k)-xcn{k}(:,indc_prox))'*...
            (xn(:,k)-xcn{k}(:,indc_prox)));
...
% figure
% plot(un,yn,'k*')
% plot(xcn{end}(1,:),xcn{end}(2,:),'g*')
toc
figure
plot(dados_(1,:),dados_(2,:),'c*',xcn{end}(1,:),xcn{end}(2,:),'r*',centros_(1,:),centros_(2,:),'k*')
% hold on
% xm = (-0.1:0.005:1.1)';
% ym = (-0.1:0.005:1.1)';
% for l = 1:R
